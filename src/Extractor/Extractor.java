...
class Extractor{
	...
	for(IClass c:cha){
	    if (c.getClassLoader().getName().toString().equals("Application")){
	...
	for(IClass c:cha){
		...
		for(IMethod m : methods){
			...
			while(iriterator.hasNext()){
				...
				String calledmethod = ((SSAAbstractInvokeInstruction) currentinstruction).getDeclaredTarget().toString();
				String calleeklassloader = calledmethod.substring(2,calledmethod.indexOf(','));
				if (calleeklassloader.equals("Application")){
